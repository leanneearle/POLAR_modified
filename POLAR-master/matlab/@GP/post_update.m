...
if ~isempty(obj.strong_pref_data)
    % go through each pairwise preference
    for i = 1:size(obj.strong_pref_data,1)
        label = obj.strong_pref_labels(i,:); %preference label

        % if a normal preference feedback: 
        if label == 1 || label == 2
            s_pos = obj.strong_pref_data(i,label); %preferred index
            s_neg = obj.strong_pref_data(i,3-label); %non-preferred index
        ...
        elseif label == 11 || label == 22
            label = round(label/10); % 11 = 1, 22 = 2
            s_pos = obj.strong_pref_data(i,label); %preferred index
            s_neg = obj.strong_pref_data(i,3-label); %non-preferred index
...
%             % strong preference method 1: reduce noise
%             % calculate feedback likelihood
%             z = (f(s_pos) - f(s_neg)) ./ (obj.pref_noise* 0.01); %link function input
%             switch obj.linkfunction
%                 case 'sigmoid'
%                     objective = objective - sum(log(obj.sigmoid(z)));
%                 case 'gaussian'
...
%             %strong preference method 2: repeat strong prefs
%             % calculate feedback likelihood
%             z = (f(s_pos) - f(s_neg)) ./ (obj.pref_noise); %link function input
%             w = 0; % initialize count for number of repeats
%             switch obj.linkfunction
%                 case 'sigmoid'
%                     while w < 100
%                         objective = objective - sum(log(obj.sigmoid(z)));
...
if ~isempty(obj.strong_pref_data)
    for i = 1:size(obj.strong_pref_data,1)
        label = obj.strong_pref_labels(i,:);
...
%             z = (f(s_pos) - f(s_neg)) ./ (obj.pref_noise * 0.01);
%             switch obj.linkfunction
%                 case 'sigmoid'
%                     grad_i_terms = 1./(obj.pref_noise * 0.01) .* (obj.sigmoid_der(z) ./ obj.sigmoid(z) );
%                 case 'gaussian'
%                     grad_i_terms = 1./(obj.pref_noise * 0.01) .* (normpdf(z) ./ normcdf(z) );
%             end
% 
%             % method 2: repeat strong prefs
%             % calculate feedback likelihood
%             z = (f(s_pos) - f(s_neg)) ./ (obj.pref_noise); %link function input
%             w = 0; % initialize count for number of repeats
%             switch obj.linkfunction
...
%             % method 1: reduct noise
%             z = (f(s_pos) - f(s_neg)) ./ (obj.pref_noise * 0.01);
%             switch obj.linkfunction
%                 case 'sigmoid'
%                     sigmz = obj.sigmoid(z);
%                     sigmz(sigmz == 0) = 10^(-100);
%                     first_i_terms = (obj.sigmoid_der2(z) ./ (obj.pref_noise*0.01)^2 )./sigmz;
%                     second_i_terms = ( (obj.sigmoid_der(z)./ (obj.pref_noise*0.01)) ./ sigmz ).^2;
%                     final_i_terms = -  (first_i_terms - second_i_terms);
%                 case 'gaussian'
%                     ratio = normpdf(z) ./ normcdf(z);
%                     final_i_terms = (ratio .* (z + ratio)) ./ ((obj.pref_noise*0.01).^2);
%             end
%             % method 2: repeat strong preferences
%             z = (f(s_pos) - f(s_neg)) ./ (obj.pref_noise);
%             w = 0; % initialize the number of repeats
%             switch obj.linkfunction
%                 case 'sigmoid'
%                     while w < 100
%                         % calculations
