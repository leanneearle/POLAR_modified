...
%
% Output:
%     1) obj.mean: (# flattened actions)x1 vector of the posterior mean
%     (ie. estimated reward)
%     2) obj.sigma: (# flattened actions)x(# flattened actions) matrix of
%     the poster covariance matrix
%     3) obj.uncertainty: (# flattened actions)x1 vector of the uncertainty
%     at each point of the reward 
%%%
...
options = optimoptions(@fminunc,'Algorithm','trust-region',...
    ...
    'Display', 'off'); %'iter-detailed');  %'off'); % -- NORTHEASTERN: uncomment to see full details of optimization --
...
% % % Strong Preference Feedback - method 1: reduce the noise
% if ~isempty(obj.strong_pref_data)
%     % calculate feedback likelihood
%     % will make noise essentially zero if strong preference
%     z = (f(obj.strong_pref_pos_ind) - f(obj.strong_pref_neg_ind)) ./ (obj.pref_noise .* (0.01).^obj.extra_pref); %link function input
%     switch obj.linkfunction
%         case 'sigmoid'
%             objective = objective - sum(log(obj.sigmoid(z)));
%         case 'gaussian'
...
% Strong Preference Feedback - method 2: repeat strong preferences (done in
% vectorize feedback)
if ~isempty(obj.strong_pref_data)
   
    % calculate feedback likelihood
    % will make noise essentially zero if strong preference
    z = (f(obj.strong_pref_pos_ind) - f(obj.strong_pref_neg_ind)) ./ (obj.pref_noise); 
    switch obj.linkfunction
        case 'sigmoid'
            objective = objective - sum(log(obj.sigmoid(z)));
        case 'gaussian'
...
% % Strong Preference Feedback - method 1: reduce noise
% if ~isempty(obj.strong_pref_data)
%     z = (f(obj.strong_pref_pos_ind) - f(obj.strong_pref_neg_ind)) ./ (obj.pref_noise .* (0.01).^obj.extra_pref);
%     switch obj.linkfunction
%         case 'sigmoid'
%             grad_i_terms = 1./obj.pref_noise .* (obj.sigmoid_der(z) ./ obj.sigmoid(z) );
%         case 'gaussian'
...
%     
%     % vectorized addition/subtraction of terms
%     grad(obj.strong_pref_pos_unique_inds) = grad(obj.strong_pref_pos_unique_inds) - obj.strong_pref_pos_repeating_inds'*grad_i_terms;
%     grad(obj.strong_pref_neg_unique_inds) = grad(obj.strong_pref_neg_unique_inds) + obj.strong_pref_neg_repeating_inds'*grad_i_terms;
% end


% Strong Preference Feedback - method 2: repeat strong preferences (done in vectorizeFeedback)
if ~isempty(obj.strong_pref_data)
    z = (f(obj.strong_pref_pos_ind) - f(obj.strong_pref_neg_ind)) ./ (obj.pref_noise); 
    switch obj.linkfunction
        case 'sigmoid'
...
%     end
%     % Vectorized addition of Hessian terms
%     Lambda(obj.strong_pref_pospos_unique_inds) = Lambda(obj.strong_pref_pospos_unique_inds) + obj.strong_pref_pospos_repeating_inds'*final_i_terms;
%     Lambda(obj.strong_pref_negneg_unique_inds) = Lambda(obj.strong_pref_negneg_unique_inds) + obj.strong_pref_negneg_repeating_inds'*final_i_terms;
%     Lambda(obj.strong_pref_posneg_unique_inds) = Lambda(obj.strong_pref_posneg_unique_inds) - obj.strong_pref_posneg_repeating_inds'*final_i_terms;
%     Lambda(obj.strong_pref_negpos_unique_inds) = Lambda(obj.strong_pref_negpos_unique_inds) - obj.strong_pref_negpos_repeating_inds'*final_i_terms;
% end

%%% method 2: repeat strong prefereneces (done in VectorizeFeedback.m)
if ~isempty(obj.strong_pref_data)
    z = (f(obj.strong_pref_pos_ind) - f(obj.strong_pref_neg_ind)) ./ (obj.pref_noise);
    switch obj.linkfunction
        case 'sigmoid'
