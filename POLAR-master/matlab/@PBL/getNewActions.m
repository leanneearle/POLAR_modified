...
% Description: Draws new actions to query using @Sampling class
% Inputs:
...
if any(obj.settings.feedback.types == 4)
    if length(compared_actions_visited) > 1
        ...
        obj.iteration(iteration).feedback.sp_x_subset = comparisons_visited; % -- NORTHEASTERN --
        obj.iteration(iteration).feedback.sp_x_full = comparisons_global; % -- NORTHEASTERN --
    else
        obj.iteration(iteration).feedback.sp_x_subset = []; % -- NORTHEASTERN --
        obj.iteration(iteration).feedback.sp_x_full = []; % -- NORTHEASTERN --
    end
end
% --------- NORTHEASTERN -----------------------------------------------
