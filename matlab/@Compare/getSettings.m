...
%   default_settings = setting object containing default settings
%   setting_struct = setting object containing defined settings
    ...
    switch all_params{1}
        case 'feedback_types'
            ...
            switch num2str(setting_struct.(all_params{1}){i})
                ...
                case '4'   % -- NORTHEASTERN --
                    setting_labels{i} = 'strong pref only';  % -- NORTHEASTERN --
                case num2str([1,2])
                ...
                % --------- NORTHEASTERN -------------------------------
                case num2str([2,4])
                    setting_labels{i} = 'strong pref + coac';
                case num2str([3,4])
                    setting_labels{i} = 'strong pref + ord';
                case num2str([2,3, 4])
                    setting_labels{i} = 'strong pref + coac + ord';
                % --------- NORTHEASTERN -------------------------------
            end
        ...
        case 'simulated_ord_noise'
            ...
            setting_labels{i} = sprintf('%s = %1.2f',all_params{1},setting_struct.(all_params{1}){i});
        % --------- NORTHEASTERN -------------------------------
        case 'simulated_strong_pref_threshold'
            new_settings.simulation.simulated_strong_pref_threshold = setting_struct.(all_params{1}){i};
            setting_labels{i} = sprintf('%s = %1.2f',all_params{1},setting_struct.(all_params{1}){i});
        % --------- NORTHEASTERN -------------------------------
        case 'n'
