...
% -------- NORTHEASTERN --------------------------------------------------
%   7) strong_pref_data: qx2 matrix with comparison indices referencing the
%           action_list
%   8) strong_pref_labels: qx1 column vector indicating 0 for no pref, 1 for
%           preferred first action, 2 for preferred second action, 11 for
%           strongly preferred first action, 22 for strongly preferred
%           second action
% -------- NORTHEASTERN --------------------------------------------------
...
obj.previous_data.strong_preference = struct('x_subset',[],'x_full',[],'y',[]);   % --- NORTHEASTERN --
...
% --------------- NORTHEASTERN ------------------------------------------
obj.previous_data.strong_preference.x_subset = reshape(visitedInds(strong_pref_data),[],2);
obj.previous_data.strong_preference.x_full = reshape(globalInds(strong_pref_data),[],2);
obj.previous_data.strong_preference.y = strong_pref_labels;
% --------------- NORTHEASTERN ------------------------------------------
