...
% Description: Begin a new experiment using the settings loaded in obj.settings
% Inputs:
%    obj = preference-based learning algorithm object
%    plotting flag = flag for plotting, either 0 or 1
%    isSave = flag for saving, either 0 or 1
%    exportfile_path = string of path to export to
...
while true
    ...
    if feedback.preference == -1 % adding here didn't work
        obj.iteration(end) = [];
    ...
    % -------- NORTHEASTERN -------------------------------------------
    if feedback.strong_preference == -1
        obj.iteration(end) = [];
        if length(obj.post_model) == iter
            obj.post_model(iter) = [];
        end
