...
% Description: Updates posterior over points_to_include which are updated to
% be obj.posterior_actions. 
% Inputs:
%    obj = preference-based learning algorithm object
%   points_to_include = vector containing at which flattened action points
%   to evaluate the posterior over (depends on if using a subset or not)
%   globalInds = n x 1 matrix of the sampled global indices
%   iteration = iteration #
...
if ~isempty(obj.previous_data)
    if strcmp(model.which,'subset')
        ...
        strong_pref_data = obj.previous_data.strong_preference.x_subset;  % -- NORTHEASTERN --
    else
        ...
        strong_pref_data = obj.previous_data.strong_preference.x_full;  % -- NORTHEASTERN --
    end
    ...
    strong_pref_labels = obj.previous_data.strong_preference.y;  % -- NORTHEASTERN --
    strong_pref_data(strong_pref_labels == 0,:) = [];  % -- NORTHEASTERN --
    strong_pref_labels(strong_pref_labels == 0) = [];  % -- NORTHEASTERN --
else
...
if any(obj.settings.feedback.types == 4)
    if strcmp(model.which,'subset')
        strong_pref_data = cat(1,strong_pref_data,obj.feedback.strong_preference.x_subset);
    else
        strong_pref_data = cat(1,strong_pref_data,obj.feedback.strong_preference.x_full);
    end
    strong_pref_labels = cat(1,strong_pref_labels,obj.feedback.strong_preference.y);
end
% ---------------- NORTHEASTERN -------------------------------------
...
isVectorized = 1; % NORTHEASTERN: KEEP VECTORIZED
