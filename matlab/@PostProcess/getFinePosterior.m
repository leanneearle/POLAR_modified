...
% Description: Get posterior using finer discretizations
% Inputs:
...
if ~isempty(alg.previous_data)
    ...
    strong_pref_data = alg.previous_data.strong_preference.x_full; % -- NORTHEASTERN -- 
    pref_labels = alg.previous_data.preference.y;
    coac_labels = alg.previous_data.coactive.y;
    ord_labels = alg.previous_data.ordinal.y;
    pref_labels = alg.previous_data.strong_preference.y;  % -- NORTHEASTERN --
else
...
if any(alg.settings.feedback.types == 4)
    strong_pref_data = cat(1,strong_pref_data,alg.feedback.strong_preference.x_full);
    strong_pref_labels = cat(1,strong_pref_labels,alg.feedback.strong_preference.y);
end
% ------------ NORTHEASTERN ----------------------------------------------
...
if ~isempty(newGlobalIndMapping)
   ...
   strong_pref_data = newGlobalIndMapping(strong_pref_data);  % -- NORTHEASTERN ---
