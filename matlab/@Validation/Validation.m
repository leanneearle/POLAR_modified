classdef Validation < handle
    ...
    methods
        function obj = Validation(alg,num_trials)
            ...
            switch alg.settings.sampling.type
                case 1  % regret minimization + thompson sampling
                    obj.enterValidationOptimization(alg,num_trials);
                case 2  % active learning + information gain
    ...
    methods (Static)
        ...
        function preference = getUserPreference
            preference = input('Which action do you prefer? (1,2 or 0 for no preference):   ');
            % ----------- NORTHESTERN-------------------------------------
            if any([preference == 11, preference == 22])
                disp('No strong preferences (11 or 22) are required or accepted in the validation phase')
            end
            % ----------- NORTHESTERN------------------------------------
